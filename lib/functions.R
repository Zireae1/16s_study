...
#примениитьь lapply с readqiime
...
LoadSixProject <- function(patway)
{
  family <- UniteMatrices(ReadQiimeSumFeats(pathway$group.one$fam.group.one.otu.tbl),
                          ReadQiimeSumFeats(pathway$group.three$fam.group.three.otu.tbl))
  family <- UniteMatrices(ReadQiimeSumFeats(pathway$group.five$fam.group.five.otu.tbl),family)
  family <- UniteMatrices(ReadQiimeSumFeats(pathway$group.six$fam.group.six.otu.tbl),family) 
  
  genus <- UniteMatrices(ReadQiimeSumFeats(pathway$group.one$gen.group.one.otu.tbl),
                          ReadQiimeSumFeats(pathway$group.three$gen.group.three.otu.tbl))
  genus <- UniteMatrices(ReadQiimeSumFeats(pathway$group.five$gen.group.five.otu.tbl),genus)
  genus <- UniteMatrices(ReadQiimeSumFeats(pathway$group.six$gen.group.six.otu.tbl),genus) 
  
  species <- UniteMatrices(ReadQiimeSumFeats(pathway$group.one$spe.group.one.otu.tbl),
                         ReadQiimeSumFeats(pathway$group.three$spe.group.three.otu.tbl))
  species <- UniteMatrices(ReadQiimeSumFeats(pathway$group.five$spe.group.five.otu.tbl),species)
  species <- UniteMatrices(ReadQiimeSumFeats(pathway$group.six$spe.group.six.otu.tbl),species) 
  
  otu <- UniteMatrices(ReadQiimeOtuTableNoTax(pathway$group.one$otu.group.one.tbl),
                       ReadQiimeOtuTableNoTax(pathway$group.three$otu.group.three.tbl))
  otu <- UniteMatrices(ReadQiimeOtuTableNoTax(pathway$group.five$otu.group.five.tbl),otu)
  otu <- UniteMatrices(ReadQiimeOtuTableNoTax(pathway$group.six$otu.group.six.tbl),otu) 
  
  meta.table <- rbind(read.csv(pathway$group.one$meta.group.one.csv, stringsAsFactors=F),
                      read.csv(pathway$group.three$meta.group.three.csv, stringsAsFactors=F))
  meta.table <- rbind(read.csv(pathway$group.five$meta.group.five.csv, stringsAsFactors=F),meta.table)
  meta.table <- rbind(read.csv(pathway$group.six$meta.group.six.csv, stringsAsFactors=F),meta.table)
  
  otup <- 100 * otu / rowSums(otu)
  #ToDo(Anna) create checking
  list(family=family, genus=genus, species=species, otu=otu, otup=otup, meta=meta.table)
}

### Print Mean and Stdev for group of samples
PrintMeanAndStd<-function(fam.gen.spe.listTOP, min.trsh){
  ...
  for (i in 1:length(MeansTOP)){
    ...
    WriteTable (ResultMeanAndSd, pathway$group.one$outdir, paste("tableTOP_g1_",i)) 
  }
  ### TO DO: print for one sample
}

PrintMeanAndStd2<-function(fam.gen.spe.listTOP, min.trsh){
  MeansTOP <- lapply(fam.gen.spe.listTOP, colMeans) 
  SdTOP <- lapply(fam.gen.spe.listTOP, function(x){apply(x,2,sd)})
  for (i in 1:length(MeansTOP)){
    ResultMeanAndSd<-cbind(round(MeansTOP[[i]][which(MeansTOP[[i]]>=min.trsh)],digits=2), 
                           round(SdTOP[[i]][which(MeansTOP[[i]]>=min.trsh)], digits=2))
    colnames(ResultMeanAndSd)<-c("Mean, %", "Stdev, %")
    WriteTable (ResultMeanAndSd, pathway$group.one$outdir, paste("tableTOP_g2_",i)) 
  }
  ### TO DO: print for one sample
}
