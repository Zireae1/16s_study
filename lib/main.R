...
constants <- lapply(ReadIni(path.to.constants), function(x) lapply(x, as.numeric))
constants$constants$perc
...
table.top <- list(group.one = TopFeatures(total.table, type = "Type.1", 
                                          subset = "case", 
                                          perc = constants$constants$perc),
                  group.two = TopFeatures(total.table, type = "Type.1", 
                                          subset = "control", 
                                          perc = constants$constants$perc))
# как то применить allaply+uniteMatrices для элеменотов table.top
...
alphaDivgroup.one <- (total.table$alpha.div[
  which(rownames(total.table$alpha.div) 
...
MDS(ddHSN, pathway$group.one$GraphsDir, 
    total.table$family[which(rownames(total.table$family)
                             %in% total.table$meta[which(total.table$meta[,"Type.1"]%in% "case"),"samples_name"]),])
